#!/usr/bin/env bash
#
# demo-raw.sh — Launch the full onion routing system with raw TCP (no TLS).
#
# Starts:  discovery (port 8080)
#          entry relay (port 9001) — raw TCP
#          middle relay (port 9002) — raw TCP
#          exit relay (port 9003) — raw TCP
#          tor-client SOCKS5 proxy (port 1080) — raw TCP
#
# Usage:
#   bash scripts/demo-raw.sh
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo "Launches the raw TCP onion routing demo on localhost."
            echo "Options:"
...
# ---- Build (raw TCP = --no-default-features) ----
if [ "$SKIP_BUILD" = false ]; then
    section "Building (raw TCP, --no-default-features)"
...
start_relay() {
    ...
    section "Starting $node_type relay (port $port) — raw TCP"
...
# ---- Tor Client ----
section "Starting Tor Client (SOCKS5 on port $SOCKS_PORT) — raw TCP"

echo "  Launching tor-client with TUI dashboard..."
echo "  (Press 'q' or Ctrl+C to stop everything)"
echo ""
echo "  Mode:       Raw TCP (no TLS)"
echo "  Discovery:  127.0.0.1:$DISCOVERY_PORT"
echo "  Entry:      127.0.0.1:$ENTRY_PORT  (raw TCP)"
echo "  Middle:     127.0.0.1:$MIDDLE_PORT (raw TCP)"
echo "  Exit:       127.0.0.1:$EXIT_PORT   (raw TCP)"
